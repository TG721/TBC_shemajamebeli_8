...
class MainFragmentViewModel @Inject constructor(private val repository: MyRepository): ViewModel() {
    private val _myState =
        MutableStateFlow<ResponseState<List<Content>>>(ResponseState.Empty()) //mutable state flow
    val myState: StateFlow<ResponseState<List<Content>>> = _myState //immutable state flow
